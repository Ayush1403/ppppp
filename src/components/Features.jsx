...
const Features = ({projects}) => {
  ...
  return (
   ...
   <div className='h-fit w-full lg:w-1/2 relative'>
   ...
   <Link to={project.link} className='w-full'>
     <div className='w-full h-screen flex flex-col justify-center items-center bg-surface relative backdrop-blur-xl'>
      <div className='w-full h-full absolute' style={{backgroundColor: project.accentColor, filter: "blur(120px)", opacity: 0.04}}></div>
 <h1  ref={(el) => (titleRefs.current[index] = el)} className='text-2xl heading font-main uppercase absolute text-primary/10 tracking-wider text-[clamp(2rem,20vw,100vw)] font-black z-10'>{project.title}.{project.title}.{project.title}.</h1>
 ...
 <div className='w-15 h-15 border-b-3 border-r-3 absolute bottom-20 right-20 border-accent'></div>
  <h1 className='absolute top-20 left-20 text-white/20 font-main font-black text-4xl'>Click TO See Demo</h1>
